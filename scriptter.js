const charpstarVTO = {
	...
	createPopup() {
		...
		canvasV.className = "charpstarVTO-video-canvas";
		canvasV.id = "charpstar-canvas-video";
		...
		// Create Glass canvas element
		var canvasG = document.createElement("canvas");
		canvasG.className = "charpstarVTOC-glasses-canvas";
		canvasG.id = "charpstar-canvas-glasses";
		canvasG.style.height = "100%";
		canvasG.style.position = "absolute";
		canvasG.style.zIndex = "10002"; // To overlap the video element
		canvasG.style.transform = "scaleX(-1)";
		...
		videoWrapper.appendChild(canvasG);
		...
		this.sceneManager = new SceneManager(canvasV, canvasG, true, this.useOrtho);
...
class Glasses {
	constructor(scene, width, height, environmentMap, renderer) {
		...
		this.xRotSmoother = new Smoother(0.5, 0);
		this.yRotSmoother = new Smoother(0.5, 0);
		...
		return new Promise((res, rej) => {
			...
			loader.load(
				...
				(gltf) => {
					...
					this.scaleFactor = size.x * 0.95; // Increase the size by 20%
	...
	updateFaceOccluder(occluder) {
		...
		occluder.position.set(
			midEyes.x,
			midEyes.y,
			midEyes.z - 40, // Adjust this value to change the occluder's distance from the face
		);
...
class SceneManager {
	constructor(canvasV, canvasG, debug = false, useOrtho = true) {
		this.canvasV = canvasV;
		this.canvasG = canvasG;
		...
		this.glassRenderer = new THREE.WebGLRenderer({
			canvas: this.canvasG,
			devicePixelRatio: window.devicePixelRatio || 1,
			context: canvasG.getContext("webgl2"),
			antialias: true,
			alhpa: true
		});


		this.glassRenderer.outputEncoding = THREE.sRGBEncoding;
		this.glassRenderer.physicallyCorrectLights = true;
		this.glassRenderer.toneMapping = THREE.ACESFilmicToneMapping;
		this.glassRenderer.toneMappingExposure = 0.6;
		...
		const renderScene = new THREE.RenderPass(this.glassScene, this.camera);
		...
		//	bloomPass.radius = 1;
	 	...
	 	this.composer = new THREE.EffectComposer(this.glassRenderer);
	...
	async loadEnvironmentMap() {
		...
		this.glassScene.background = envMap;
		this.glassScene.environment  = envMap;
		this.buildLights();
		this.buildGlasses();
	...
	buildVideoBg() {
		...
		this.videoBg = new VideoBackground(this.videoScene,
			this.videoRenderer.domElement.width,
			this.videoRenderer.domElement.height
		);
		
		console.log (this.videoScene);
	}


	buildGlasses() {
		charpstarVTO.glassClass = new Glasses(this.glassScene, this.glassRenderer.domElement.width, this.glassRenderer.domElement.height, this.environmentMap, this.glassRenderer)
	}
	...
	buildOrthoCamera() {
		this.camera = new THREE.OrthographicCamera(
			-this.videoRenderer.domElement.width / 2,
			this.videoRenderer.domElement.width / 2,
			this.videoRenderer.domElement.height / 2,
			-this.videoRenderer.domElement.height / 2,
			-2000,
			2000
		)
	...
	resizeRendererToDisplaySize() {
		const canvasV = this.videoRenderer.domElement;
		const canvasG = this.glassRenderer.domElement;
		// match dimension of canvas with
		// dimension of video
		if (this.videoWidth != canvasV.clientWidth ||
			this.videoHeight != canvasV.clientHeight) {
		...
		const width = canvasV.clientWidth | 0;
		const height = canvasV.clientHeight | 0;
		const needResize = canvasV.width !== width || canvasV.height !== height;
		if (needResize) {
			this.videoRenderer.setSize(width, height, false);
			this.glassRenderer.setSize(width, height, false);
		}
		return needResize;
	}
	...
	animate() {
		...
		if (this.resizeRendererToDisplaySize()) {
			charpstarVTO.glassClass.updateDimensions(
				this.glassRenderer.domElement.width,
				this.glassRenderer.domElement.height
			);
		...
		// render scene
		//this.glassRenderer.render(this.glassScene, this.camera);
