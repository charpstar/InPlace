...
class SceneManager {
	constructor(canvasV, canvasG, debug = false, useOrtho = true) {
		...
		this.scene = new THREE.Scene(); // for the video background
		this.glassesScene = new THREE.Scene(); // for the glasses
		...
		this.glassesRenderer = new THREE.WebGLRenderer({
			canvas: this.canvasG,
			antialias: true,
			alpha: true
		});
		this.glassesRenderer.setPixelRatio(window.devicePixelRatio);
		this.glassesRenderer.setSize(window.innerWidth, window.innerHeight);
		this.glassesRenderer.outputEncoding = THREE.sRGBEncoding;

		this.glassesRenderer.physicallyCorrectLights = true;
		this.glassesRenderer.toneMapping = THREE.NoToneMapping;
		this.glassesRenderer.toneMappingExposure = 1;
		...
		this.buildCamera();
		this.buildControls();
		this.buildVideoBg(); // make sure this adds to `this.scene`
		this.loadEnvironmentMap(); // make sure this adds to `this.glassesScene`

		const renderScene = new THREE.RenderPass(this.scene, this.camera);
		const glassesRenderScene = new THREE.RenderPass(this.glassesScene, this.camera); // for the glasses renderer
		this.composer = new THREE.EffectComposer(this.glassesRenderer);
		this.composer.addPass(renderScene);
		this.composer.addPass(glassesRenderScene);
	...
	async loadEnvironmentMap() {
		...
		this.glassesScene.background = envMap;
		this.glassesScene.environment = envMap;
		this.buildLights();
		this.buildGlasses();
	...
	buildGlasses() {
		charpstarVTO.glassClass = new Glasses(this.glassesScene, this.glassesRenderer.domElement.width, this.glassesRenderer.domElement.height, this.environmentMap, this.glassesRenderer)
	}

	buildLights() {
		// Add an ambient light
		const ambientLight = new THREE.AmbientLight(0xffffff, 0.4);
		this.glassesScene.add(ambientLight); // Add light to the glassesScene

		// Add a directional light
		const pointLight = new THREE.PointLight(0xffffff, 0.1);
		pointLight.position.set(0, 50, 100);
		this.glassesScene.add(pointLight); // Add light to the glassesScene
	}
	...
	animate() {
		...
		if (this.resizeRendererToDisplaySize()) {
			charpstarVTO.glassClass.updateDimensions(
				this.glassesRenderer.domElement.width,
				this.glassesRenderer.domElement.height
			);
		...
		// render glasses
		this.glassesRenderer.render(this.glassesScene, this.camera);
	}
...
class VideoBackground {
	...
	createNewPlane() {
		console.log("Creating geometry with: ", this.width, this.height);
